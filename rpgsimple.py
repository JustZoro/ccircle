...
class Player(Character):
    ...
    "%s's health: %d/%d" % (self.name, self.health, self.health_max)

    def tired(self):
        print
        "%s feels tired." % self.name
        self.health = max(1, self.health - 1)

    def rest(self):
        if self.state != 'normal': print
        "%s can't rest now!" % self.name;
        self.enemy_attacks()
        else: print
              "%s rests." % self.name
        if randint(0, 1):
...
def flee(self):
    ...
    if randint(1, self.health + 5) > randint(1, self.enemy.health):
        ...
        print
    "%s couldn't escape from %s!" % (self.name, self.enemy.name);
    self.enemy_attacks()


def attack(self):
    if self.state != 'fight': print
...
def enemy_attacks(self):
    if self.enemy.do_damage(self): print
    "%s was slaughtered by %s!!!\nR.I.P." % (self.name, self.enemy.name)
...
print
"%s enters a dark cave, searching for adventure." % p.name
...
while (p.health > 0):
    ...
    if len(args) > 0:
        ...
        if not commandFound:
            print
            "%s doesn't understand the suggestion." % p.name